#!/usr/bin/env python3
"""Create label-separated Ada-RCA source bundles from RCAEval datasets."""
...
def build_manifest(profile, raw_path: str, output_root: Path):
    result = load_cases(profile, raw_path)
    ...
    dataset_root = Path(raw_path).resolve()
...
def main():
    ...
    parser.add_argument("--re2ob-root", required=True, help="path to the RCAEval RE2-OB directory")
    parser.add_argument("--re2tt-root", required=True, help="path to the RCAEval RE2-TT directory")
    parser.add_argument("--output-root", required=True, help="directory for generated source bundles")
    args = parser.parse_args()
    root = PROJECT_ROOT / args.output_root
    raw_paths = {"re2ob": args.re2ob_root, "re2tt": args.re2tt_root}
    summaries = {
        profile.key: build_manifest(profile, raw_paths[profile.key], root)
        for profile in (RE2OB, RE2TT)
    }
